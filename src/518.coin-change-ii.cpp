...
public:
    ...
    //创建dp数组：
    vector<vector<unsigned long long int>> dp(amount+1,vector<unsigned long long int>(coins.size(),0));
    //dp[amount][count]
    //初始化 init
    ...
    for(int i=1;i<dp.size();++i){  //amount
        for(int j=1;j<dp[i].size();++j){  //coins
            ...
            else if(coins[j] == i) dp[i][j] = dp[i][j-1]+1;
            else dp[i][j] = dp[i][j-1] + dp[i-coins[j]][j];
        }
    }
    // print(dp);
